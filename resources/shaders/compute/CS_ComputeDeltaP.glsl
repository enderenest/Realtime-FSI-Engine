...
float computeSCorr(vec3 dpos) {
    if (ubo.enableSCorr == 0u) return 0.0;

    float w = calcPoly6Kernel(dpos);
    if (ubo.wq <= 1e-12 || w <= 0.0) return 0.0;

    float ratio = w / ubo.wq;
    if (ratio <= 0.0) return 0.0;

    return -ubo.kCorr * pow(ratio, ubo.nCorr);
}
...
void main() {
        ...
        for (int y = -1; y <= 1; y++) {
            for (int x = -1; x <= 1; x++) {
                ...
                for (int k = startEnd.x; k < startEnd.y; k++) {
                    ...
                    float lambda_j = solver[j].predPos_lambda.w;

                    vec3 rij = xi - xj;
                    vec3 gradW = calcGradSpikyKernel(rij);

                    float scorr = computeSCorr(rij);
                    dp += gradW * (lambda_i + lambda_j + scorr);
                }
            }
        }
    }
    ...
    // STORE DELTA P
    // =========================================================================
    solver[id].deltaP_rho.xyz = dp * ubo.invRho0;
}
